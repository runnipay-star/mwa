...
export const Cart: React.FC<CartProps> = ({ user }) => {
  ...
  // Calcolo totale con logica fedeltà
  // NOTA: Questa logica è puramente visuale per il frontend. 
  // Il backend ricalcolerà il totale reale per sicurezza.
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await createCheckoutSession(courseIds, user.id, user.email || '');
      
      if (response && response.url) {
        // Opzionale: pulire il carrello prima di reindirizzare, ma Stripe potrebbe fallire/annullare.
        // Meglio lasciarlo pieno finché non siamo sicuri (o svuotarlo sul successo).
        // Per semplicità qui non svuotiamo, l'utente vedrà i corsi acquistati nella dashboard.
        clearCart(); 
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8 sticky top-32">
              ...
              <p className="text-center text-xs text-gray-400 mt-4">
                Pagamento sicuro SSL criptato tramite Stripe.
              </p>
            </div>
          </div>
